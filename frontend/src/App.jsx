...
function AppContent() {
  ...
  const [hasShownInitialPreferences, setHasShownInitialPreferences] =
    useState(false);
  const [showDetailPopup, setShowDetailPopup] = useState(false);
  const [popupItem, setPopupItem] = useState(null);

  // Check if user has already seen preferences modal (for existing users)
  const hasUserSeenPreferences = localStorage.getItem("hasSeenPreferences");
  ...
  const checkAndShowPreferencesModal = async () => {
    ...
    const hasSeenBefore = localStorage.getItem("hasSeenPreferences");
    if (hasSeenBefore) {
      setHasShownInitialPreferences(true);
      return;
    }
    ...
    if (
      ...
      // Even if preferences exist, mark as seen
      setHasShownInitialPreferences(true);
      localStorage.setItem("hasSeenPreferences", "true");
    }
  };

  // Reset isNewUser flag when preferences modal is closed
  const handlePreferencesModalClose = () => {
    setShowPreferencesModal(false);
    // Mark that user has seen preferences modal
    localStorage.setItem("hasSeenPreferences", "true");
    // Reset the new user flag after showing preferences modal
    if (isNewUser) {
      resetNewUserFlag();
      setHasShownInitialPreferences(true);
    }
